...
import {
  canEditBishopTraining,
  getDefaultBishopPayload,
  mergeBishopPayload,
  type BishopSlide,
  type BishopSlidePresentation,
  type BishopTopic,
  type BishopTrainingPayload,
  type BishopOfficialResourceRow,
} from "@/lib/training/bishop-training"

const STORAGE_KEY = "stake-app-bishop-training-checklist"
const PAYLOAD_DRAFT_PREFIX = "stake-app-bishop-training-payload-"

const inputClass =
  "w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-gray-900 focus:outline-none focus:ring-teal-500 focus:border-teal-500 text-sm"
...
function scrollToSlide(slideId: string) {
  document.getElementById(`bp-slide-${slideId}`)?.scrollIntoView({ behavior: "smooth", block: "start" })
}
...
function loadPayloadDraftFromBrowser(stakeId: string): BishopTrainingPayload | null {
  try {
    const raw = localStorage.getItem(payloadDraftKey(stakeId))
    if (!raw) return null
    return mergeBishopPayload(JSON.parse(raw))
  } catch {
    return null
  }
}

function savePayloadDraftToBrowser(stakeId: string, payload: BishopTrainingPayload) {
  try {
...
export default function BishopTrainingPage() {
  ...
  const [payload, setPayload] = useState<BishopTrainingPayload>(getDefaultBishopPayload)
    ...
    ;(async () => {
      try {
        ...
        if (user) {
          ...
          if (!cancelled) setCanEdit(canEditBishopTraining(profile?.role as string | undefined))
        }

        if (!sid) {
          if (!cancelled) {
            setPayload(getDefaultBishopPayload())
            setBaselineJson(JSON.stringify(getDefaultBishopPayload()))
        ...
        if (error) {
          console.warn("bishop_training_content:", error.message, error)
          ...
          if (draft) {
            ...
            setDbNoticeTone("info")
            setDbNote(
              "Could not reach the Supabase table (often not migrated yet). Loaded your last draft from this browser. You can keep editing and use Save — it will store locally until `047_bishop_training_content.sql` is applied, then Save will sync to the stake."
            )
            return
          }
          const merged = getDefaultBishopPayload()
          setPayload(merged)
          ...
          if (isLikelyMissingTableError(error)) {
            setDbNoticeTone("info")
            setDbNote(
              "The `bishop_training_content` table is not in your database yet. Showing defaults. Run `047_bishop_training_content.sql` from this repo in the Supabase SQL Editor, then refresh. Until then, Save stores a draft on this device only."
            )
          } else {
        ...
        const merged = mergeBishopPayload(data?.payload)
        setPayload(merged)
        setBaselineJson(JSON.stringify(merged))
  ...
  const cancelEdit = () => {
    try {
      setPayload(JSON.parse(baselineJson) as BishopTrainingPayload)
    } catch {
      setPayload(getDefaultBishopPayload())
    }
    setEditMode(false)
  ...
  const savePayload = async () => {
    ...
    try {
      ...
      setDbNoticeTone("info")
      setDbNote(
        `Could not save to Supabase (${msg}). Your changes were saved in this browser. After you run \`047_bishop_training_content.sql\` in the SQL Editor, open this page again and Save once to copy the draft to the database for all users.`
      )
    } finally {
      setSaving(false)
    }
  ...
  const updateTopic = (id: string, patch: Partial<BishopTopic>) => {
    setPayload((p) => ({
      ...p,
  ...
  const updateResource = (id: string, patch: Partial<BishopOfficialResourceRow>) => {
    setPayload((p) => ({
      ...p,
  ...
  const updateSlidePresentation = (patch: Partial<BishopSlidePresentation>) => {
    setPayload((p) => ({
      ...p,
  ...
  const updateDeckSlide = (id: string, patch: Partial<BishopSlide>) => {
    setPayload((p) => ({
      ...p,
      slidePresentation: {
        ...p.slidePresentation,
  ...
  const addDeckSlide = () => {
    ...
    const slide: BishopSlide = { id, title: "New slide", body: "" }
    setPayload((p) => ({
      ...p,
      slidePresentation: {
        ...p.slidePresentation,
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <Link
        ...
        className="text-sm text-teal-600 hover:text-teal-800 inline-flex items-center gap-1 mb-4"
      >
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Bishop Training</h1>
            {editMode ? (
              <textarea
      ...
      <Card className="mb-6 overflow-hidden border-teal-200 shadow-sm">
        <CardHeader className="border-b border-teal-100 bg-gradient-to-r from-teal-50/90 to-white">
              ...
              <div className="min-w-0 flex-1">
                {editMode ? (
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full border border-teal-200 bg-white px-2.5 py-0.5 text-xs font-medium text-teal-800">
        ...
        <CardContent className="space-y-4 pt-6">
          {!editMode && (
            <div className="flex flex-col gap-3 rounded-xl border border-teal-100 bg-gradient-to-br from-slate-50 to-teal-50/40 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-0.5 max-w-xl">
                    Expect about 30–45 minutes to read every module once. New bishops: prioritize modules 1–7 before
                    your first major membership or welfare decision if possible.
                  </p>
                </div>
              </div>
            </div>
          ...
          {editMode ? (
            ...
            <p className="text-sm text-gray-600 whitespace-pre-wrap border-l-2 border-teal-200 pl-4">
              ...
              <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto pr-1">
                {payload.slidePresentation.slides.map((s, i) => (
                  <button
                    ...
                    className="inline-flex max-w-full items-center gap-1 rounded-full border border-gray-200 bg-white px-2.5 py-1 text-left text-xs font-medium text-gray-800 shadow-sm transition hover:border-teal-300 hover:bg-teal-50"
                  >
                    <span className="shrink-0 text-teal-600 tabular-nums">{i + 1}</span>
            ...
            {payload.slidePresentation.slides.map((slide, idx) => (
              ...
              >
                <div className="flex shrink-0 flex-col items-center gap-1">
                  <div className="flex h-11 w-11 items-center justify-center rounded-full bg-teal-600 text-sm font-bold text-white shadow">
                    {idx + 1}
                  </div>
                  {editMode && (
