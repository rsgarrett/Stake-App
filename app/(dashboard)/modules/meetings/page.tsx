...
import { Calendar, List, ArrowLeft, Plus, Clock, MapPin, ExternalLink, Repeat, Edit, User } from "lucide-react"
...
export default function MeetingsPage() {
  ...
  const [agendaItems, setAgendaItems] = useState<Record<string, { title: string; assigned_to?: string | null; item_order: number }[]>>({})
  ...
  const loadAgendaItems = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select("meeting_id, title, assigned_to, item_order")
      ...
      const grouped: Record<string, { title: string; assigned_to?: string | null; item_order: number }[]> = {}
      data?.forEach((item) => {
        if (!grouped[item.meeting_id]) grouped[item.meeting_id] = []
        grouped[item.meeting_id].push(item)
      })
  ...
  const handleDateClick = (date: Date) => {
    setListDayFilter(date)
    setViewMode("list")
  }

  const handleEventClick = (event: { id: string; meeting_type: string; [key: string]: any }) => {
  ...
  const dayListMeetings = listDayFilter
    ? meetings.filter((m) => isSameDay(new Date(m.scheduled_date), listDayFilter))
    : []

  const listViewMeetings = listDayFilter ? dayListMeetings : upcomingMeetings
    ...
    <div className="p-6">
      ...
      {viewMode === "calendar" ? (
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {listDayFilter
                    ? `${listViewMeetings.length} meeting${listViewMeetings.length !== 1 ? "s" : ""} scheduled`
                    : "Next 10 scheduled meetings"}
                </p>
              </div>
            </div>
          ...
          {loading ? (
          ...
          ) : listViewMeetings.length > 0 ? (
            <div className="space-y-3">
              {listViewMeetings.map((meeting) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 px-4 py-3">
                      ...
                      {/* Agenda items / assignments */}
                      {items.length > 0 && !schedDetail && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
            ...
            <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 py-12 text-center">
              <p className="text-gray-500">
                {listDayFilter ? "No meetings on this day" : "No upcoming meetings"}
              </p>
              <Button
                variant="outline"
                size="sm"
